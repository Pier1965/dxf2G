...
void cut_arco(float dx, float dy, float alfa);
...
double pi;
// x,y,zerr errore di posizione, x,y,z, posizione teorica, xyzr reale
	...
	while((opt=getopt(argc, argv, "n:p:d:v:u:f:i:e:h")) !=-1){
		switch(opt){
			...
			case 'v':
				...
				break;
			case 'u':
				strncpy(unita,optarg,strlen(optarg));
				printf("(Units = %s)\n", unita);
				break;
			case 'f':
				if(strlen(optarg)<255){
					strncpy(nome_file_comp,optarg,strlen(optarg));
					printf("(File name = %s)\n", nome_file_comp);
				}
				else
					printf("\nFile name too long\n");
			...
			case 'e':
				if(strlen(optarg)<12){
					strncpy(modalita,optarg,strlen(optarg));
					printf("(Path mode = %s)\n", modalita);
				}
				else{
...
void crea_file_ordinato(void){      // inizio funzione
	...
	float xd, yd, zd, alfa_quarti_next;
					...
					for(;;){
						...
						if(strcmp(strinp,"10")==0){
							...
							if(strncmp(strinp,"42",2)==0)
								...
								fakeint = fscanf(input_file,"%f",&alfa_quarti_next);  // WEB changed to fakeint = fscanf to fix warning about first parameter ???
							else
				...
				if((strcmp(strinp,"10")==0)&(nvp!=0)){
					...
					for(i=0;i<nvp;i++){
						...
						if(strncmp(strinp,"42",2)==0)
							// se si allora leggo la tang dell'angolo/4 dell'arco
							// con cui raggiungere il verice che segue
							fakeint = fscanf(input_file,"%f",&alfa_quarti_next);
						else
							// altrimenti non c'Š raccordo
							alfa_quarti_next=0;
						// scrivo i dati del vertice nel file temporaneo di taglio
...
void taglia(void){
	...
	*cnc_file='\0';  // WEB fixed mkissing cast warning
	strcpy(cnc_file,nome_file_comp);
	strncat(cnc_file,".ngc", strlen(".ngc"));
...
void cut_polyline(int colore, int volte){
	...
	float xd, yd, zd, dx, dy, dz, x0, y0, z0, alfa_quarti_now, alfa_quarti_next;
	float xip, yip, zip, alfa_quarti_next_ip, zattuale;
	...
	if(tagliare==1){
		...
		dz=-pp-zr;
		z=0;
		alfa_quarti_now=alfa_quarti_next;
		for(j=0;j<volte;j++){
			...
			for(i=1;i<nvp;i++){
				...
				if(alfa_quarti_now==0){
					...
					// arco
					cut_arco(dx,dy,alfa_quarti_now);
				}
				alfa_quarti_now=alfa_quarti_next;
			}
			...
			if (j!=volte-1){
				...
				fakeint = fscanf(temp_file,"%f",&alfa_quarti_next_ip);
				alfa_quarti_now=alfa_quarti_next_ip;
...
void cut_lwpolyline(int colore, int volte){ 
	...
	float xd, yd, dx, dy, dz, x0, y0, alfa_quarti_now, alfa_quarti_next;
	float xip, yip, alfa_quarti_next_ip, zattuale;
	...
	for(;;){// inizio for scansione lwpolilinea ATTENZIONE... per evitare casini con codici e numeri 
		...
		if((strcmp(strinp,"10")==0)&(nvp!=0)){
			for(i=0;i<nvp;i++){  // inizio for scansione interna vertice
				...
				if(strncmp(strinp,"42",2)==0){ // può essere 42, 10, EOF, o altro
					...
					alfa_quarti_next=0;// altrimenti non c'Š raccordo
					// scrivo i dati del vertice nel file temporaneo di taglio
				fprintf(temp_file,"%f\n%f\n%f\n",xd,yd,alfa_quarti_next);
			}
			strcpy(stringa,strinp);
		...
		if(strcmp(strinp,"70")==0){ // solo fuori da vertex
			...
			// dovro' aggiungere un vertice coinc
			// col primo nel file di taglio temporaneo
			fprintf(log_file, "Rilevata lwpolyline chiusa line 3366\n");
		}
		if (strncmp(strinp,"EOF",3)==0){
	...
	if(tagliare==1){
		...
		for(j=0;j<volte;j++){  // inizio ciclo da ripetere per volte=np passate
			...
			for(i=1;i<nvp;i++){ // inizio scansione vertici della polilinea
				...
				dx=xd-xr;
				dy=yd-yr;
				if(alfa_quarti_now==0){ // segmento di retta?
					fprintf(output_file_cnc,"G1 X%f Y%f\n", xd, yd);
					sposta_punta(dx,dy,0,vt,1);
				}
				else {                  // arco
...
void cut_arco(float dx, float dy, float alfa){
	...
	float popf, r, xm, ym, xc, yc, delta, d, dalfa, alfapart, beta;
	float xpi, ypi, dxi, dyi, xarc, yarc;
	int i, nseg, orario;
	
	xarc=yarc=0;					// coordinate relative PO origine arco
	alfa=4*atan(alfa);				// angolo di apertura dell'arco in radianti
	...
	dalfa=res_arc*sqrt(resx*resx+resy*resy)/r;
	if(orario==1){
		if(abso_float(alfa)<pi){
			xc=xm+d*sin(delta);			// coordinate centro arco orario
			yc=ym-d*cos(delta);
...
void evaluate_layer_name(void){
	...
	if(strlen(strinp)==26) {
		strncpy(layer_name,strinp,strlen(strinp));
